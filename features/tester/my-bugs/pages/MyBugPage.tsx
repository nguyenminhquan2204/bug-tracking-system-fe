...
import { useManageBugTesterStore } from "../stores/useManageBugTesterStore";
...
export function MyBugPage() {
  ...
  const { bugList, getMyBugsForTester } = useManageBugTesterStore(
    useShallow((state) => ({
      getMyBugsForTester: state.getMyBugsForTester,
  ...
  useEffect(() => {
    const fetchApi = async () => {
      await Promise.all([getMyBugsForTester()]);
    };
    fetchApi();
  }, []);
  ...
  return (
    <>
      <BugDetail selectedBug={selectedBug} setSelectedBug={setSelectedBug} flash={'tester'} />
